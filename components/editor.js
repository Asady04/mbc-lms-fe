...
/*functions*/
export default function JoditEditorReact() {
  const editor = useRef(null); //declared a null value 
  const [content, setContent] = useState("Lorem ipsum dolor sit amet"); //declare using state
  ...
  const config = useMemo( //  Using of useMemo while make custom configuration is strictly recomended 
    () => ({              //  if you don't use it the editor will lose focus every time when you make any change to the editor, even an addition of one character
      ...
      uploader: {
        ...
        imagesExtensions: ['jpg', 'png', 'pdf', 'gif', 'svg', 'webp'] // this line is not much important , use if you only strictly want to allow some specific image format
      },
    }),
    []
  );
  ...
  return (
    <>
      ...
      <main>
        <div className="w-[100%] md:px[8.33%] lg:px-[12.5%] xl:px-[16.67%] 2xl:px-[25%] flex items-center flex-col">
          <div className="flex flex-col items-center">
            ...
            <div className='text-center'>visit <a href="https://www.soubhagyajit.com/blogs/adding-Jodit-js-:-rich-text-editor-to-a-react-(next.js)-application" target='_blank' className="text-blue-500">
              www.soubhagyajit.com</a> for more information
            </div> */}
          </div>
          <div>
