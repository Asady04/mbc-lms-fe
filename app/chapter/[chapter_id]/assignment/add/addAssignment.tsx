...
export default function AddAssignment(props: any) {
  ...
  const handleSubmit = async () => {
    try {
      ...
      const formattedOpen = open.toDate(timeZone).toLocaleString("sv-SE").replace("T", " ");
      const formattedDue = due.toDate(timeZone).toLocaleString("sv-SE").replace("T", " ");
  
      console.log("Sending data:", { title, content, type, open: formattedOpen, due: formattedDue, chapter_id });
  ...
  return (
    <div className="text-left text-default-600 p-5 mt-2 space-y-3">
      ...
      <div className="mt-2 space-y-3">
        <Button color="primary" onPress={handleSubmit}>Add</Button>
      </div>
    </div>
  );
}
